...
export async function POST(request) {
  try {
    const { intervenantId, email, prenom, nom, role, seniorName, phone } = await request.json()
    ...
    if (phone) {
      const whatsappTo = 'whatsapp:' + phone.replace(/\s/g, '').replace(/^0/, '+33')
      const message = `Bonjour ${prenom} 👋\n\nVous avez été invité(e) sur *Holiris* pour le suivi de *${seniorName}* en tant que ${role}.\n\n🔑 Votre code d'accès : *${code}*\n\n1. Créez votre compte sur https://holiris.fr/login\n2. Entrez ce code pour activer votre espace\n\n⚠️ Partagez uniquement l'état général et le moral. Jamais de diagnostics ou ordonnances.\n\n_Holiris — Prendre soin de ceux qui nous sont chers_`
